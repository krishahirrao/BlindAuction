🕵️ Secret Auction Program (Python)

A simple command-line based Secret Auction system built using Python. This program allows multiple users to place bids privately and determines the highest bidder at the end.
 
📌 Features
👤 Multiple bidders can participate
💰 Accepts bid amounts from each user
🔒 Keeps bids hidden between participants
🧠 Automatically finds the highest bidder
🧩 Beginner-friendly Python logic
⚙️ How It Works
User enters their name
User enters their bid amount
Program asks if there are more bidders
If yes → screen clears (simulated)
If no → winner is displayed
The program prints the highest bidder and bid amount
🧑‍💻 Code Overview
Uses a dictionary to store bids (name: bid)
A function find_highest_bidder() is used to:
Loop through all bids
Compare values
Find the highest bidder
🚀 How to Run
Make sure Python is installed

Clone this repository:

git clone https://github.com/your-username/your-repo-name.git

Navigate to the project folder:

cd your-repo-name

Run the program:

python main.py
📂 Project Structure
secret-auction/
│── main.py
│── README.md
⚠️ Note

The screen clearing is simulated using:

print("\n" * 50)
It may not fully hide previous bids depending on the terminal.
🎯 Learning Outcomes
Working with functions
Using loops & conditionals
Understanding dictionaries in Python
Basic user input handling
📌 Future Improvements
Add real screen clearing (os.system('cls'/'clear'))
Add input validation
Create a GUI version
Store bids in a file
🙌 Author

Krish Ahirrao
